"""
S004 — Frieren-only dialogue from panel balloons (`002.jpg` row 2 left).

Story: panel_s004jap.png (JP) + panel_s004.png (EN layout).
  Fern: フリーレン様。 (not muxed)
  Frieren: また大陸魔法協会からの依頼？

Default: WAV only under outputs/voice/S004/. Use --mux for Kling overlay.

Usage:
  cd scripts
  python generate_s004_dialogue.py --skip-clone --language Japanese --tag frieren_dialogue_v1_ja
  python generate_s004_dialogue.py --mux --video ..\\outputs\\video\\final\\S004_kling....mp4
"""
...
from qwen_tts import (
    ...
    FRIEREN_S004_DIALOGUE_START_SEC,
    FRIEREN_S004_LANGUAGE,
    FRIEREN_S004_PHRASES,
    FRIEREN_S004_PHRASES_EN,
    FRIEREN_S004_PROMPT,
    FRIEREN_S004_PROMPT_EN,
    load_reference_text,
    load_registry,
    resolve_character_embedding,
    synthesize_s004_frieren,
)


def _s004_line_and_prompt(language: str) -> tuple[str, str]:
    if language.lower() in ("english", "en"):
        return FRIEREN_S004_PHRASES_EN[0], FRIEREN_S004_PROMPT_EN
    return FRIEREN_S004_PHRASES[0], FRIEREN_S004_PROMPT
...
def main() -> int:
    parser = argparse.ArgumentParser(description="S004 Qwen3 Frieren dialogue (WAV default)")
    ...
    parser.add_argument("--start-sec", type=float, default=FRIEREN_S004_DIALOGUE_START_SEC)
    parser.add_argument("--duck", type=float, default=0.35)
    parser.add_argument(
        "--language",
        choices=("Japanese", "English"),
        default=FRIEREN_S004_LANGUAGE,
    )
    ...
    line_text, default_prompt = _s004_line_and_prompt(args.language)
    ...
    stem = out_dir / f"s004_frieren_{ts}.mp3"

    wav_path = out_dir / f"s004_frieren_{tag}_{ts}.wav"
    synthesize_s004_frieren(
        emb_url,
        stem,
        wav_path,
