...
S007_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s007.png`** (**layered memory composite** — **ghost Frieren** upper, **solid Stark** + **Fern** below at camp), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **vertical layering** and **scale**: **translucent Frieren** in the **upper field**, **Stark** and **Fern** **seated in the lower camp plane**, **Fern** holding the **grimoire** — **same left-right positions** as the manga, **do not mirror** horizontally. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **daytime** forest light; **small fire** optional lower-corner **accent** only — echo stays **soft and separate** from the solid figures. "
)

S007_EDIT_PROMPT = S007_EDIT_LEAD_IN + S007_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S007 → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s007.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
