"""
S004 — ByteDance **Seedance 2.0** image-to-video (`bytedance/seedance-2.0/image-to-video`).

**Stage_02:** **`present`** **MS** **B2** — Frieren **foreground side profile** with **open grimoire**;
Fern **deeper** with **sealed envelope** (depth separation, not flat two-shot). **PREFIX_PRESENT** cool Northern camp.

Default driver still matches **`generate_s004_kling_i2v.py`** (`Tests/Final/` Nano Banana edit).

**Policy note:** S004 is MS two-face — Seedance often returns `generated_video` sensitive content even with §4b prompts.
Prefer **`generate_s004_kling_i2v.py`**; this script defaults to **near-static** motion (see anime-scene-i2v-prompting §4b).
...
DEFAULT_START = (
    ROOT / "Tests" / "Final" / "S004_nano-banana-2-edit_20260327T104245Z.png"
)
...
# Plain text only. S004 = MS two-face (high Seedance policy risk) — default near-static per skill §4b.
NEAR_STATIC_MOTION_PROMPT = (
    "Fantasy anime television production shot, PG adventure, locked-off medium shot, 35mm soft grain, "
    "2.39:1, cool Northern forest camp, overcast diffused light, same composition as the reference still. "
    "Foreground figure with open illustrated book: only page edge flutter and hair and coat drift in breeze, "
    "seated side profile, no turn to camera, no mouth motion. "
    "Background figure with sealed paper packet: nearly frozen, slight scarf movement only, stays in depth, "
    "no step forward, no interaction between figures. "
    "Environmental wind in leaves only. No cuts, no on-screen text, no manga texture, no fourth wall."
)

DEFAULT_MOTION_PROMPT = NEAR_STATIC_MOTION_PROMPT

MOTION_PROMPT = DEFAULT_MOTION_PROMPT
...
def main() -> int:
    parser = argparse.ArgumentParser(description="S004 → Seedance 2.0 image-to-video")
    ...
    parser.add_argument(
        "--full-motion",
        action="store_true",
        help="Use fuller MS motion (higher policy risk; not default after S004 failures)",
    )
    parser.add_argument(
        "--prompt",
        default=None,
        help="Override motion prompt (default: near-static S004)",
    )
    ...
    elif args.full_motion:
        prompt_text = (
            "Fantasy anime television production shot, PG adventure, locked-off medium shot, 35mm soft grain, "
            "2.39:1, cool Northern forest camp, overcast diffused light, same layout as the reference still. "
            "Foreground figure side profile with open illustrated book: subtle page movement, hair and coat drift, "
            "seated, does not turn to camera. "
            "Background figure with sealed paper packet: minimal stillness, slight scarf movement, stays in depth. "
            "Tripod-locked, no zoom, wind in leaves, one continuous beat, no cuts, no text, no fourth wall."
        )
    else:
        prompt_text = MOTION_PROMPT.strip()
