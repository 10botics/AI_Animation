...
**Same prompt** for every backend: `S016_EDIT_LEAD_IN` + `S016_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s016.png`** (JP balloons: `panels/jap/panel_s015jap.png`). Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s016.png"
...
S016_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s016.png`** (**forest meet** — trio waist-up in woodland), transform it into one finished **fantasy anime television** illustration. "
    ...
    "Refine ink into full **cel-shaded anime** with **cool Northern forest daylight**, readable **brown beard and gold monocle** on Denken, **red coat** on Stark, **white coat and scarf** on Frieren. "
)

S016_EDIT_PROMPT = S016_EDIT_LEAD_IN + S016_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S016 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\eng\\panel_s016.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
