"""
S015 — **present** WS forest meet (`panel_s015jap.png` / `004.jpg` row 2).

Panel crop id **s015** — Denken L, Stark mid, Frieren R. Prompt body shared with S016 (`S016_PROMPT_FLUX`).

- **Default: Nano Banana 2** `fal-ai/nano-banana-2/edit`.
- **Legacy:** `--model flux-2-pro-edit`.

Default ref: **`panels/eng/panel_s015.png`**. JP: **`panels/jap/panel_s015jap.png`**.

Usage:
  cd scripts
  python generate_s015_ref_edit.py
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s015.png"
...
S015_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s015.png`** (**forest meet** — trio waist-up in woodland), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **composition**: **Denken on the left**, **Stark in the center**, **Frieren on the right** — **same camera height, subject scale, and left-right order** as the manga crop — **do not mirror** horizontally. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine greyscale ink into full **cel-shaded anime** with **cool Northern forest daylight**; "
    "**Denken** — warm **brown** beard and scalp hair (not white or silver), **gold monocle** on chain; "
    "**Stark** — **red** winter coat, **cream** lapels; **Frieren** — **white** coat, **thick pale scarf**, **silver-white** twin tails. "
)

S015_EDIT_PROMPT = S015_EDIT_LEAD_IN + S016_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S015 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\eng\\panel_s015.png or pass --ref panels\\jap\\panel_s015jap.png",
            file=sys.stderr,
        )
        return 1
        ...
        json.dumps(
            {
                ...
                "panel_jap": "panels/jap/panel_s015jap.png",
                "seed": args.seed,
                **meta_extra,
            },
            indent=2,
