...
S011_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s011.png`** (**Stark** **medium close** reaction beat — **gazing left** at **endless gilt** landscape), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **framing and chirality**: **only Stark** in sharp focus, **eyes and posture toward the LEFT** of the frame at the **golden** vista — **do not mirror** horizontally, **no** Fern or Frieren, **no** second warrior. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **bright** specular **gold** depth and clear **Stark** costume locks (**red coat**, **cream** lapels, **axe** on back). "
)

S011_EDIT_PROMPT = S011_EDIT_LEAD_IN + S011_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S011 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s011.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
