"""
S002 — Weise **forest camp EST** SFX via **CassetteAI only** (`cassetteai/sound-effects-generator`).

Scene ref: stage_02 S002 + fal_common S002_PROMPT_FLUX — trio, small campfire, axe & packs, cool Northern day.

- Finds `outputs/video/final/*s002*` (case-insensitive) for ffprobe ambient length.
- Writes `outputs/audio/sfx/S002/cassetteai/*.wav` (or other ext from URL).
- Manifest: `outputs/fal/S002_sfx_cassette_<ts>.json`

Usage:
  cd scripts
  python generate_s002_sfx.py
"""
...
S002_JOBS: tuple[CassetteJob, ...] = (
    CassetteJob(
        ...
        prompt=(
            ...
            "dry leaves and earth, cool air, very sparse distant bird, peaceful travelers rest stop, "
            "no voices, no music, no urban noise"
        ),
        default_seconds=15,
    ),
    CassetteJob(
        ...
        prompt=(
            ...
            "not a huge bonfire, no voices, no music"
        ),
        default_seconds=8,
    ),
    CassetteJob(
        ...
        prompt=(
            "Pure paper-and-binding foley only: soft rustle as heavy book pages turn slowly, crisp paper edge, spine creak if any, "
            "close mic ASMR-style, unmanned object — no implied reader, no breathing, no mouth sounds. "
            "Strictly no human voice, whisper, mumble, humming, reading aloud, dialogue, or music — only dry paper and book materials"
        ),
        default_seconds=5,
    ),
    CassetteJob(
        ...
        prompt=(
            ...
            "people seated still, no footsteps walking, no voices"
        ),
        default_seconds=5,
    ),
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        dest="only_slug",
        metavar="SUBSTRING",
        help="Run a single job whose slug contains this text (case-insensitive), e.g. book_pages",
    )
    ...
    jobs = S002_JOBS
    if args.only_slug:
        needle = args.only_slug.strip().casefold()
        jobs = tuple(j for j in S002_JOBS if needle in j.slug.casefold())
        if not jobs:
            print(f"No job slug contains {args.only_slug!r}. Known: {[j.slug for j in S002_JOBS]}", file=sys.stderr)
            return 1
    ...
    meta: dict = {
        ...
        "only_slug_filter": args.only_slug,
        "generations": [],
    }
    ...
    suffix = f"_{args.only_slug.replace(' ', '_')}" if args.only_slug else ""
    meta_path = meta_dir / f"{SHOT_ID}_sfx_cassette_{ts}{suffix}.json"
