"""
S005 — **present** CU: **Fern** + **Lernen** memory telegraph (`002.jpg` bottom tier, viewer’s right).

**Same prompt** for every backend: `S005_EDIT_LEAD_IN` + `S005_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s005.png`**. Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s005.png"
...
S005_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s005.png`** (Weise camp **letter** beat), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **composition**: **Fern** foreground with **letter or paper**, **soft memory portrait** of elder mage **Lernen** behind her — **same figure scale and left-right layout** as the manga, **do not mirror** horizontally. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **Northern forest** colour and depth — **one live Fern**, Lernen read stays **ethereal / flashback**, not a second solid body beside her. "
)

S005_EDIT_PROMPT = S005_EDIT_LEAD_IN + S005_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S005 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s005.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
