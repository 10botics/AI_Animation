"""
S002 — **present** Northern Weise **forest camp** trio (default ref: `panels/panel_s002.png`).

**Same prompt** for every backend: `S002_EDIT_LEAD_IN` + `S002_PROMPT_FLUX`.

- **Default: Nano Banana 2** `fal-ai/nano-banana-2/edit` — `--aspect-ratio`, `--resolution`.
- **Legacy:** `fal-ai/flux-2-pro/edit` — `--model flux-2-pro-edit`; `landscape_16_9` / `--image-size`.
- **`--model both`** — one upload, runs Flux then Nano Banana (same ref, same timestamp prefix on outputs).
...
DEFAULT_REF = ROOT / "panels" / "panel_s002.png"
...
# FLUX.2 /edit: lead-in first — layout anchor; strip manga UI and dialogue boxes (BFL + project skill).
S002_EDIT_LEAD_IN = (
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S002 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    parser.add_argument(
        ...
        help="Edit backend: Flux (default), Nano Banana, or both in one run",
    )
    parser.add_argument(
        "--ref",
        type=Path,
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s002.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
