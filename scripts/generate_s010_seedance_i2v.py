"""
S010 — ByteDance **Seedance 2.0** image-to-video (`bytedance/seedance-2.0/image-to-video`).

**WS HERO** present golden ridge: three travelers **seen from behind**, tiny vs gilt Weise vista —
traveler left, mage center with rectangular trunk, warrior right with axe on back. §4b production prompt.

Default still: `Tests/Final/S010_nano-banana-2-edit_20260326T094807Z.png`.
...
DEFAULT_START = (
    ROOT / "Tests" / "Final" / "S010_nano-banana-2-edit_20260326T094807Z.png"
)
...
MOTION_PROMPT_SILENT = (
    "Fantasy anime television production shot, PG adventure, locked-off extreme wide shot, 35mm soft grain, "
    "2.39:1, bright daytime ridge overlook, clear blue sky, same layout as the reference still. "
    "Three travelers small in frame, seen from behind facing away toward distant fortified city and golden valley — "
    "traveler figure left, mage figure center holding one rectangular brown travel trunk at side, "
    "warrior figure right with large axe on back — left-to-right order unchanged, do not mirror, "
    "no turn toward camera, no frontal faces, figures stay small versus the vista. "
    "Subject motion subtle only: gentle breeze on pigtails, long hair, scarf, and coat hems; trunk strap still; "
    "no walking out of stance, no large gestures. "
    "Environment: polished-gold trees, terrain, and citadel walls with slow specular glints; soft valley haze drifts; "
    "clouds move slowly. "
    "Camera tripod-locked with imperceptible slow reverent push toward the citadel only, "
    "no whip pan, no orbital move, no crane rise, no 3D fly-through. "
    "One continuous awe beat, no cuts, no on-screen text, no manga texture, no fourth wall, no new people."
)

AUDIO_TAIL = (
    " Ambient ridge overlook: gentle panoramic wind in high grass, airy vast valley space, soft distant tree hiss, "
    "subtle gold-valley air shimmer, very light cloth and hair flutter on figures seen from behind; "
    "no speech, no dialogue, no vocalizations, no singing, no music score."
)


def main() -> int:
    parser = argparse.ArgumentParser(description="S010 → Seedance 2.0 I2V (golden ridge WS HERO)")
    parser.add_argument(
        "--start-image",
        type=Path,
    ...
    parser.add_argument(
        ...
        default="5",
        help='Fal duration string (default "5"; try "10" for slower vista)',
    )
    parser.add_argument(
        "--resolution",
        choices=("480p", "720p"),
