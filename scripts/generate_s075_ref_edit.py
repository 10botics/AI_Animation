...
- **Default:** `fal-ai/nano-banana-2/edit` — `--aspect-ratio 16:9`, `--resolution 1K|2K|…`.
- **Legacy:** `fal-ai/flux-2-pro/edit` — `landscape_16_9` or `--image-size 1280x720` / `1920x1080`.

Pass `--ref` to override; full `016.jpg` still mixes flashback tiers — prefer the panel crop.
...
S075_EDIT_LEAD_IN = (
    ...
    "The reference wins **composition**: **preserve horizontal orientation** — **Frieren stays on the left side of frame**, **Denken's back on the right** (over-the-shoulder), same as the upload; **do not mirror** the scene left-right. "
    "Keep **snowfield foreground**, **rock-streaked snowy ridges** beyond, and low **overcast** sky band; same mountain layering and horizon height as the panel. "
    "Following text refines anime paint, cool light, and fabric only; it must not swap biome, camera angle, or **flip** the layout. "
    "**Denken** remains **back-facing** nearer camera on the **right** (**brown** elder head hair, high dark collar); **Frieren** on the **left** with **thick scarf** and winter coat — **present** Northern travel. "
    "Remove ink halftone, panel borders, gutters, speech balloons, and lettering; only the painted scene remains. "
)

S075_EDIT_PROMPT = S075_EDIT_LEAD_IN + S075_PROMPT_FLUX
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="S075 panel → image edit (Flux 2 Pro or Nano Banana 2); same prompt string.",
        epilog="Nano Banana 2: fal-ai/nano-banana-2/edit — see https://fal.ai/models/fal-ai/nano-banana-2/edit/api",
    )
    parser.add_argument(
        "--model",
        dest="model_choice",
        ...
        help="Edit backend (default: nano-banana-2-edit)",
    )
    parser.add_argument(
        "--ref",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Flux 2 Pro only: output 16:9 preset or fixed WxH",
    )
    parser.add_argument(
        "--aspect-ratio",
        dest="aspect_ratio",
        default="16:9",
        help='Nano Banana 2 only (default "16:9"). See Fal schema for other ratios.',
    )
    parser.add_argument(
        "--resolution",
        dest="resolution",
    ...
    if args.model_choice == "flux-2-pro-edit":
        if args.image_size == "1280x720":
            resolved_size: dict[str, int] | str = {"width": 1280, "height": 720}
        elif args.image_size == "1920x1080":
            resolved_size = {"width": 1920, "height": 1080}
        else:
            resolved_size = "landscape_16_9"

        base_args: dict = {
            "prompt": S075_EDIT_PROMPT,
        ...
        models = [(FLUX_PRO_EDIT_SLUG, FLUX_PRO_EDIT_MODEL_ID, base_args)]
        meta_extra = {"image_size": resolved_size}
    else:
        base_args = {
            "prompt": S075_EDIT_PROMPT,
        ...
        models = [(NANO_BANANA_2_EDIT_SLUG, NANO_BANANA_2_EDIT_MODEL_ID, base_args)]
        meta_extra = {
            "aspect_ratio": args.aspect_ratio,
            "resolution": args.resolution,
        }
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s075.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
