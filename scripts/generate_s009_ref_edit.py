"""
S009 — **present** WS: **Stark**, **Frieren**, **Fern** **walking** the **forest path** toward camera — **Frieren** carries **rectangular case** (ref `panels/eng/panel_s009.png`).

**Same prompt** for every backend: `S009_EDIT_LEAD_IN` + `S009_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s009.png`**. Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s009.png"
...
S009_EDIT_LEAD_IN = (
    "Using the uploaded reference (**daylit forest path** — **Stark**, **Frieren** with **travel case**, **Fern** — **`panel_s009.png` or an S009 color still**), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **who**, **where**, **wardrobe**, and **left-to-right** order **Stark – Frieren – Fern** — **do not mirror**. "
    "**Authorized refinement for motion / video prep:** **break** flat **symmetrical** march posing — **stagger** feet and **offset** stride timing; **asymmetric** shoulders and **micro** head turns; **slightly** shift **camera** (**lower** eyeline + **gentle** tilt **or** mild **three-quarter** path read) so depth and **parallax** improve **without** losing the trio cluster. "
    "**Strip** any **mage staff** from **Fern** if present — **hands relaxed**; **only Stark** has the **axe** on his back. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **bright daytime** canopy light and **dappled** ground — **Northern journey** continuity. "
)

S009_EDIT_PROMPT = S009_EDIT_LEAD_IN + S009_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S009 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s009.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
