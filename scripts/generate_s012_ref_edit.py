"""
S012 — **present** WS: **backs** toward **Fortified City of Weise** (`003.jpg` bottom tier viewer-left).

**Same prompt** for every backend: `S012_EDIT_LEAD_IN` + `S012_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s012.png`**. Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s012.png"
...
S012_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s012.png`** (**wide ridge backs** — trio toward **Fortified City of Weise**), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **composition**: **camera behind** the party — **all three backs** to viewer, **left-to-right** **Fern – Frieren – Stark**, **gilt forest and city** filling the distance — **same scale, height gaps, and silhouette read** as the manga, **do not mirror** horizontally. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **full polished-gold** environment read (continuous with **S010**), **clear blue sky**, **melancholic** lore weight — **do not** leave the city **non-metallic**. "
)

S012_EDIT_PROMPT = S012_EDIT_LEAD_IN + S012_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S012 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s012.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
