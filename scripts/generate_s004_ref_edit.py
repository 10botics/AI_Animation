"""
S004 — **present** Weise camp: **Frieren** + **Fern**, **grimoire** + sealed envelope (`002.jpg` row 2, viewer's left panel).

**Same prompt** for every backend: `S004_EDIT_LEAD_IN` + `S004_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s004.png`**. Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s004.png"
...
S004_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s004.png`** as **identity + wardrobe + locale reference**, output **one** finished fantasy anime illustration with a **deliberate camera reframe** — **not** a 1:1 copy of the panel’s lens. "
    "**Recompose:** **Frieren** in **foreground side profile / lateral camera** (clear elf-ear silhouette, **open grimoire** edge-on); **Fern** **farther back in depth** (background plane), **smaller**, envelope still toward Frieren. "
    "**Preserve** from upload: **who** they are, **Northern winter** costuming, **forest camp** setting, **two people only**, **no mirror flip** of the world. "
    "Strip halftone, panel borders, gutters, **speech balloons**, and lettering; only the painted scene remains. "
)

S004_EDIT_PROMPT = S004_EDIT_LEAD_IN + S004_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S004 panel → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s004.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
