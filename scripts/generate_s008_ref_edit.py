"""
S008 — **present** MS: **daytime** camp **grimoire deal** — Fern, Frieren, Stark (`003.jpg` top tier viewer-left).

**Same prompt** for every backend: `S008_EDIT_LEAD_IN` + `S008_PROMPT_FLUX`.
...
Default ref: **`panels/eng/panel_s008.png`**. Pass `--ref` to override.
"""
...
DEFAULT_REF = PANELS_ENG / "panel_s008.png"
...
S008_EDIT_LEAD_IN = (
    "Using the uploaded **`panel_s008.png`** (**daytime grimoire beat** — **Fern** **back to camera**, **Frieren** holds **open** book, **Stark** rear **left**), transform it into one finished **fantasy anime television** illustration. "
    "The reference wins **composition**: **exactly three** seated on **forest floor**, **Frieren** **right** with **open grimoire** and **circular seal** read, **Fern** **center-foreground** facing her — **same depth and left-right order** as the manga, **do not mirror** horizontally. "
    "**Remove** all **speech balloons**, **SFX**, **panel borders**, halftone, and lettering; only the painted scene remains. "
    "Refine into **cel-shaded anime** with **clear Northern daylight** under the canopy — **sun-flecks**, **soft shadows**; **not** a night camp scene. "
)

S008_EDIT_PROMPT = S008_EDIT_LEAD_IN + S008_PROMPT_FLUX


def main() -> int:
    parser = argparse.ArgumentParser(
        description="S008 → Flux 2 Pro and/or Nano Banana 2; same prompt string.",
    ...
    if not ref_path.is_file():
        print(
            ...
            "Add panels\\panel_s008.png or pass --ref path\\to\\crop.png",
            file=sys.stderr,
        )
        return 1
